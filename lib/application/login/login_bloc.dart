...
class LoginBloc extends Bloc<LoginEvent, LoginState> {
  ...
  LoginBloc(this._repository) : super(LoginState.initial()) {
    ...
    on<_Submit>((event, emit) async {
      ...
      var response = await _repository.login(email: state.email, password: state.password);
      response.match(
  ...
  bool get isEmailValid => RegExp(r"^[a-zA-Z0-9.a-zA-Z0-9.!#$%&'*+-/=?^_`{|}~]+@[a-zA-Z0-9]+\.[a-zA-Z]+").hasMatch(state.email);
